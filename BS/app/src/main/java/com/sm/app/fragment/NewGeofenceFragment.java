...
public class NewGeofenceFragment extends Fragment implements View.OnClickListener {
    ...
    /* check if the event is creation or updating */
    public void saveChangesInSQLiteDBLatlong(String s) {

        System.out.println("Entrato con la stringa: "+s);

        String[] arr=s.split(" ");
        //prendo ultima posizione

        String sLatt= arr[arr.length-1];
        String sLat=sLatt.replace(";","");
        String sLng= arr[arr.length-2];

        Double lat= new Double(sLat);
        Double lng= new Double(sLng);
        String name="";
        ...
        if(update)
            this.updateFenceInSqliteDB(name, name, "Geofence aggiunto dalla ricerca", "Parking", lat, lng, "500", "3333333333", "Benvenuto nel Geofence"+name, 0);
        else
            this.saveFenceInSqliteDB(name, name, "Geofence aggiunto dalla ricerca", "Parking", lat, lng, "500", "3333333333", "Benvenuto nel Geofence"+name, 0);
