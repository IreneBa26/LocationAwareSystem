...
import com.sm.app.R;
import com.sm.app.activity.MainActivity;
import com.sm.app.entity.Fence;
import com.sm.app.sqlitedb.SQLiteDBManager;
...
public class Controller {
    ...
    ***********************************************************************************************/
    ...
    public static int insertFenceOnSQLiteDB(String name, String address, String city, String province, String lat, String lng, String range, int active, String number, String textSMS, int event) {
        String db=dbManager.getIdNameFenceSQLiteDB();
        String nomeNuovo = name.replace(" ", "");
        //cancello se c'è stesso parcheggio più vecchio

        String arr[]=db.split(";");
        for (int j=0; j<arr.length-1; j++){
            String nomeArr []= arr[j].split(",");
            String app = nomeArr[0].replace(" ", "");
            System.out.println("STringaaaaaaaaaaaaaaaaa: "+app);
            int idG = new Integer(app);
            System.out.println("IDDDDDDD: "+idG);
            String nomee = nomeArr[1].replace(";", "");
            String nomeGiusto = nomee.replace(" ", "");
            System.out.println("nomeNuovo: " +nomeNuovo);
            System.out.println("nomeGiusto: " +nomeGiusto);
            if (nomeNuovo.equals(nomeGiusto)) {
                System.out.println("Caaaaaaaaaaaaaaaaaaancello");
                removeFenceFromDB(idG);
                for (int h=0; h<fences.size(); h++) {
                    if (fences.get(h).getName().equals(nomeGiusto)) {
                        fences.remove(h);
                    }
                }
        ...
        int id= dbManager.insert(name, address, city, province, lat, lng, range, active, number, textSMS, event);
